...
export default function HomeScreen(){
  ...
  const onRefresh=useCallback(()=>{
    setRefreshing(true);
    if(dirUri)doScan(dirUri).then(()=>setRefreshing(false));
    ...
    return(
      <View style={[st.card,run&&{shadowColor:color,shadowOpacity:.4,shadowRadius:12,elevation:8}]}>
        ...
        <View style={st.ca}>
          ...
          <TouchableOpacity style={[st.bi,t.status==='open'&&st.bta,miss&&st.bd]} onPress={()=>t.status==='open'?bridge.closeTunnel(p.id):bridge.openTunnel(p.id)} disabled={miss}>
            <Text style={st.bit}>🌐</Text>
          </TouchableOpacity>
        </View>
      </View>
