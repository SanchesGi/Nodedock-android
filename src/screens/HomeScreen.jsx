...
import{bridge,resolveRealPath}from '../lib/bridge';
...
export default function HomeScreen(){
  ...
  const pickDir=async()=>{
    try{
      ...
      const uri=r.uri||r;
      const realPath=resolveRealPath(uri);
      if(!realPath){
        ToastAndroid.show('Não foi possível resolver o caminho da pasta.',ToastAndroid.LONG);
        return;
      }
      setDirLabel(realPath);
      bridge.setProjectsDir(realPath);
      setTimeout(()=>bridge.refresh(),500);
    }catch(e){
      if(!DocumentPicker.isCancel(e))ToastAndroid.show('Erro ao selecionar pasta.',ToastAndroid.SHORT);
    }
  };
    ...
    return(
      <View style={[st.card,run&&{shadowColor:color,shadowOpacity:.4,shadowRadius:12,elevation:8}]}>
        ...
        <View style={st.ca}>
          ...
          <TouchableOpacity
            style={[st.bi,t.status==='open'&&st.bta,t.status==='opening'&&st.bto,miss&&st.bd]}
            onPress={()=>t.status==='open'?bridge.closeTunnel(p.id):bridge.openTunnel(p.id)}
            disabled={miss}>
            <Text style={st.bit}>🌐</Text>
          </TouchableOpacity>
        </View>
  ...
  return(
    <View style={st.root}>
      ...
      <View style={st.ga}>
        ...
        <TouchableOpacity style={st.bg} onPress={onRefresh}>
          <Text style={st.bgt}>↻ Scan</Text>
        </TouchableOpacity>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={st.empty}>
            ...
            <Text style={st.em}>
              {dirLabel?'Nenhum projeto encontrado.\nVerifique se as pastas têm package.json + server.js/index.js':'Selecione a pasta com seus projetos acima.'}
            </Text>
          </View>
        }
      />
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={st.lf}>
          ...
          {projects.map((p,i)=>(
            <TouchableOpacity key={p.id} style={[st.fb,selLog===p.id&&{borderColor:PAL[i%PAL.length]}]} onPress={()=>setSelLog(p.id)}>
              <Text style={[st.ft,selLog===p.id&&{color:PAL[i%PAL.length]}]}>
                {p.name.length>10?p.name.slice(0,10)+'…':p.name}
              </Text>
            </TouchableOpacity>
          ))}
          <TouchableOpacity style={st.fb} onPress={()=>{logsRef.current=[];setLogs([]);}}>
            <Text style={[st.ft,{color:'#ff5555'}]}>✕ Limpar</Text>
          </TouchableOpacity>
        </ScrollView>
        <ScrollView ref={scrollRef} style={st.lb} nestedScrollEnabled>
          {filteredLogs.length===0
            ?<Text style={st.le}>📡 Inicie um serviço para ver os logs.</Text>
            :filteredLogs.map(l=>{
...
const st=StyleSheet.create({
  ...
  ei:{fontSize:32,opacity:.4},
  em:{fontSize:12,color:'#3a4260',fontFamily:'monospace',textAlign:'center',lineHeight:20},
